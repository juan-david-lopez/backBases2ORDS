-- =====================================================
-- API DE ALERTAS Y REPORTES - CORREGIDO
-- Archivo: 08_alertas_reportes_api_corregido.sql
-- Propósito: Endpoints de alertas tempranas y reportes académicos
-- Ejecutar como: ACADEMICO
-- NOTA: Corregido para usar nombres de columnas reales de la BD
-- =====================================================
...
PROMPT =====================================================
PROMPT Eliminando módulo alertas anterior (si existe)
PROMPT =====================================================

BEGIN
    ORDS.DELETE_MODULE(p_module_name => 'alertas');
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('✓ Módulo alertas anterior eliminado');
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('  (No había módulo previo)');
END;
/
...
-- CORREGIDO: Usa HISTORIAL_RIESGO en lugar de columna inexistente
-- =====================================================
...
BEGIN
    ...
    FOR rec IN (
        SELECT 
            ...
            ROW_NUMBER() OVER (PARTITION BY e.cod_estudiante ORDER BY hr.fecha_evaluacion DESC) as rn
        FROM ESTUDIANTE e
...
-- CORREGIDO: Usa tabla HISTORIAL_RIESGO
-- =====================================================
...
-- CORREGIDO: Usa nombres de columnas correctos
-- =====================================================
...
BEGIN
    ...
    FROM (
        SELECT cod_estudiante, nivel_riesgo,
               ROW_NUMBER() OVER (PARTITION BY cod_estudiante ORDER BY fecha_evaluacion DESC) as rn
        FROM HISTORIAL_RIESGO
    ) WHERE rn = 1;
...
PROMPT =====================================================
PROMPT Total de endpoints de alertas: 4
