...
Write-Host "Descripcion: Obtiene todas las notas definitivas del estudiante" -ForegroundColor Gray

try {
    $response = Invoke-RestMethod -Uri "$baseUrl/estudiante/$codEstudiante" -Method Get -Headers @{Accept='application/json'} -ErrorAction Stop
...
Write-Host "Descripcion: Obtiene calificaciones de todos los estudiantes del grupo" -ForegroundColor Gray

try {
    $response = Invoke-RestMethod -Uri "$baseUrl/grupo/$codGrupo" -Method Get -Headers @{Accept='application/json'} -ErrorAction Stop
...
Write-Host "Descripcion: Obtiene el historial academico completo del estudiante" -ForegroundColor Gray

try {
    $response = Invoke-RestMethod -Uri "$baseUrl/historial/$codEstudiante" -Method Get -Headers @{Accept='application/json'} -ErrorAction Stop
    
    Write-Host "[OK] Status: 200 OK" -ForegroundColor Green
    Write-Host "`nHistorial academico:" -ForegroundColor Cyan
    ...
    Write-Host "  - Creditos aprobados: $($response.creditos_aprobados)" -ForegroundColor White
...
Write-Host "[TEST 4] Verificacion de CORS" -ForegroundColor Green
Write-Host "Descripcion: Verifica que los headers de CORS estan presentes" -ForegroundColor Gray

try {
    $response = Invoke-WebRequest -Uri "$baseUrl/estudiante/$codEstudiante" -Method Get -Headers @{Accept='application/json'; Origin='http://localhost:5173'} -ErrorAction Stop
    ...
    if ($corsHeader) {
        ...
        Write-Host "[WARNING] Access-Control-Allow-Origin: NO ENCONTRADO" -ForegroundColor Yellow
    }
} catch {
    Write-Host "[ERROR] $($_.Exception.Message)" -ForegroundColor Red
}
...
Write-Host "=========================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "[OK] Todos los endpoints de calificaciones estan funcionando correctamente" -ForegroundColor Green
Write-Host "[OK] El estudiante $codEstudiante tiene 4 asignaturas aprobadas" -ForegroundColor Green
Write-Host "[OK] El promedio acumulado es 4.3" -ForegroundColor Green
Write-Host "[OK] Los endpoints de examen/calificaciones estan listos para uso en el frontend" -ForegroundColor Green
Write-Host ""
